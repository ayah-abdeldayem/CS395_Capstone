...
def app_registration_audit(mytimer: func.TimerRequest) -> None:
    ...
    for idx, a in enumerate(all_apps, start=1):
        ...
        for c in pw_creds:
            secrets.append({
                "SecretID": c.get("keyId"),
                "SecretDescription": c.get("displayName"),
                "SecretHint": c.get("hint"),
                "SecretStartDateTime": c.get("startDateTime"),
                "SecretEndDateTime": c.get("endDateTime")
            })
